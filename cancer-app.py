...
# ---------------- [치트키] 온라인에서 나눔고딕 폰트 실시간 다운로드 ----------------
@st.cache_data
def load_font():
    font_url = "https://github.com/google/fonts/raw/main/ofl/nanumgothic/NanumGothic-Regular.ttf"
    font_path = "NanumGothic.ttf"
    # 파일이 없을 때만 다운로드하여 서버 부하 방지
...
try:
    ...
    # Matplotlib의 기본 폰트 패밀리를 나눔고딕으로 강제 지정
    plt.rcParams['font.family'] = font_prop.get_name()
except Exception as e:
    # 예외 발생 시 시스템 기본 서체로 백업
...
if st.session_state.app_step == 5:
    ...
    # font_prop가 유효한지 확인 후 맷플롯립 텍스트 요소에 수동 주입
    if 'font_prop' in locals():
        ax.set_xlabel("흡연여부 (Smoking Index)", fontproperties=font_prop)
        ax.set_ylabel("음주여부 (Alcohol Index)", fontproperties=font_prop)
        ax.set_title("다차원 군집 내 피험자 상대 좌표 매핑", fontproperties=font_prop)
